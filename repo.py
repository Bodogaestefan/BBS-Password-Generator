...
def retrieve_all_passwords(mpw, vault):
    ...
    decrypted_passwords = []
    decryption_key = utils.derive_encryption_key(mpw, salt)

    for pwd in passwords:
        decr = utils.decrypt_aes(pwd[3], decryption_key)
...
def create_new_password(vault, mpw, what_for, password, uname=None, em_addr=None):
    salt = retrieve_e_k_s(vault)
    encryption_key = utils.derive_encryption_key(mpw, salt)
    encrypted_pw = utils.encrypt_aes(password, encryption_key)
